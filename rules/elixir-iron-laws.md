...
3. **No Unconditional DB Queries in Mount** `[Enforced by: TTSR + Semantic Review]`: Hindari query berat di `mount/3` sinkron — gunakan `assign_async/3`, limit paginasi, atau cabang `connected?(socket)`. (*Rule: `rules/elixir-liveview-unbounded-query.md`*).
4. **Streams for Large Lists** `[Enforced by: TTSR + Semantic Review]`: Wajib menggunakan LiveView Streams (`stream/3`) atau `temporary_assigns` untuk daftar data hasil query guna mencegah lonjakan memori proses LiveView. (*Rule: `rules/elixir-liveview-temporary-assigns.md`*).
...
12. **Supervise All Long-Lived Processes** `[Enforced by: TTSR + Semantic Review]`: Setiap proses berumur panjang dan asynchronous wajib berada di bawah pohon pengawasan (*Supervision Tree*) atau Oban. (*Rule: `rules/elixir-no-unsupervised-task.md`*).
...
23. **Ash Resource Policy Fail-Closed (Mutlak)** `[Enforced by: TTSR + Semantic Review]`: Kebijakan otorisasi Ash wajib menolak akses secara default (*default deny/fail-closed*) dan dilarang membypass otorisasi via `authorize?: false` di kode produksi. (*Rules: `rules/elixir-ash-fail-closed.md`, `rules/elixir-ash-no-unauthorized-bypass.md`*).
